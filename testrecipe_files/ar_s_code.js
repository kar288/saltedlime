...
/* Firefox tracing */
var s_console = null;  // aliasing Firefox's console for testability
if (typeof (console) != "undefined") { s_console = console; }
var s_instanceIndex = 0;
s_FFTrace = function(theCall, theS, theSinstanceIndex) {

    if (s_console != null
        && typeof (s_console.log) == "function"
        && typeof (s_console.dir) == "function") {

        s_console.log("Omniture: s." + theCall + ", instance: " + theSinstanceIndex);
        s_console.dir({ s: theS });
    }
};

s_addFirefoxTracing = function(originalS, instanceIndex) {
    var originalT = originalS.t;
    var originalTL = originalS.tl;

    originalS.t = function() {
        s_FFTrace("t", originalS, instanceIndex);
        return originalT.apply(originalS, arguments);
    };
    
    originalS.tl = function() {
        s_FFTrace("tl", originalS, instanceIndex);
        return originalTL.apply(originalS, arguments);
    };
};

var s = s_gi(s_account);
s.charSet = "UTF-8";
s_addFirefoxTracing(s,1);
/************************** CONFIG SECTION **************************/
...
s.linkInternalFilters = "javascript:,allrecipes.com,.allrecipes.com,.allrecipes.corp"
...
function s_doPlugins(s) {
    ...
    if (s.getQueryParam('ms') == 1) {
        if (s.getQueryParam('me') == 1) {
             s.prop25=s.getQueryParam('prop25');
             s.prop26=s.getQueryParam('prop26');
             s.prop27=s.getQueryParam('prop27');
             s.prop28=s.getQueryParam('prop28');
             s.prop29 = s.getQueryParam('prop29');
             s.eVar5 = s.prop26 + '_' + s.prop27;
        }
        //Prop25 was firing when clicking a "more searches" link in the main search.
        else if (s.getQueryParam('qt') === "") {             
            s.prop25='unknown';
        }
    }
...
function s_setObjectID(o) { s_objectID = o; }
